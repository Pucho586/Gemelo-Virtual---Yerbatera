...
export const api = {
  ...
  // FASE 3: Alarms / OEE / Maintenance / Energy / Reports
  alarmsActive: () => http.get('/alarms/active').then(r => r.data),
  alarmsHistory: (limit = 100) => http.get(`/alarms/history?limit=${limit}`).then(r => r.data),
  alarmAck: (alarm_id) => http.post('/alarms/ack', { alarm_id }).then(r => r.data),
  alarmRules: () => http.get('/alarms/rules').then(r => r.data),
  alarmUpsertRule: (rule) => http.post('/alarms/rules', rule).then(r => r.data),
  alarmDeleteRule: (id) => http.delete(`/alarms/rules/${id}`).then(r => r.data),
  getOee: (window = 24) => http.get(`/oee?window_hours=${window}`).then(r => r.data),
  getMaintenance: () => http.get('/maintenance').then(r => r.data),
